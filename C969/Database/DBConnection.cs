    ...
    class DBConnection
    {
        ...
        public static List<UserAccount> GetAllUserAccounts()
        {
            ...
            try
            {
                ...
                while (reader.Read())
                {
                    UserAccount user = new UserAccount(reader.GetInt32(0), reader.GetString(1), reader.GetString(2), reader.GetInt32(3), reader.GetDateTime(4), reader.GetString(5));
                    allUsers.Add(user);
                }
        ...
        public static int CreateNewCountryInDatabase(string countryName, UserAccount u)
        {
        ...
        public static int GetCountryID(string countryName, UserAccount u)
        {
            bool countryExists = DoesCountryExist(countryName);

            if (countryExists == false)
            {
                int newID = CreateNewCountryInDatabase(countryName, u);
                return newID;
            }

            int countryID = Database.DBConnection.GetIntFromTable("countryId", "country", "country", countryName);
